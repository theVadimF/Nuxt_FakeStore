<template>
  ...
  <div class="fixed bg-white z-20 w-full max-w-xl h-full right-0 top-0 flex flex-col box-border p-3 gap-3">
    ...
    <div class="flex flex-col gap-3 h-full overflow-y-auto">
      ...
      <CartItem v-for="product in cart" :item="product"/>
    </div>
    <div class="flex items-center gap-2">
      <button :disabled="true" class="flex items-center px-3 py-1 gap-1 rounded-xl shadow-md" :class="cart.length === 0 ? 'cursor-not-allowed bg-gray-600 text-gray-200' : 'bg-green-800 text-white'"><Icon class="self-center text-xl" name="la:shopping-bag"/>Checkout</button>
